...
class NoticiaRepository(
    ...
    //esse mediador vai ficar responsavel em mediar os LiveDatas dos bancos de dados
    //tanto interno como da API
    ...
    fun buscaTodos(): LiveData<Resource<List<Noticia>?>> {
        ...
        mediador.addSource(falhasDaWebApiLiveData) { ResorceDeFalha ->
            val resourceAtual = mediador.value
            val resorceNovo: Resource<List<Noticia>?> = if (resourceAtual != null) {
                Resource(dado = resourceAtual.dado, erro = ResorceDeFalha.erro)
            } else {
                ResorceDeFalha
            }
            mediador.value = resorceNovo
        }
        return mediador
    }
